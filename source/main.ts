class Figure {
    ...
    public rotation: {point: vec2, angle: number}
    public scale: number

    private _points: vec2[]
    private _radius: number

    constructor(radius: number, vertices_count: number) {
        this._points = []
        for (let i = 0; i < vertices_count; i++) {
            let angle = i * (Math.PI / vertices_count * 2)
...
let hill = [new vec2([0, 0]), new vec2([canvas.width, canvas.height])]
let inclination = Math.asin(
    (hill[1].y - hill[0].y) / 
    (hill[1].copy().subtract(hill[0]).length())
)

let snowball = new Figure(30, 16)
snowball.scale = 1

let current_point = hill[0].copy()
...
setInterval(
    () => {
        ...
        context.beginPath()
            ...
            for (let point of snowball.points.slice(1)) {
        ...
        context.fillRect(
            snowball.position.x + snowball.radius * Math.cos(-snowball.rotation.angle), 
            snowball.position.y - snowball.radius * Math.sin(-snowball.rotation.angle),
            8, 8
        )
